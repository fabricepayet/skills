...
3. **Classify the feedback.**
   - **Correct:** the problem and requested direction are supported. When implementation is authorized, apply the smallest coherent change and verify the affected behavior; otherwise report the supported change.
   - **Directionally correct:** the problem is real but the proposed implementation is unsuitable. When implementation is authorized, fix it with the repository-appropriate approach; otherwise propose that approach and explain the deviation.
   ...
   - **Incorrect:** evidence contradicts the claim or the change would introduce greater risk. Record the specific code path, contract, or test that supports rejecting it. Reply externally only when authorized.
4. **Report disposition.** For each comment, state the classification, evidence, action taken or proposed, and verification result. Distinguish blocking correctness issues from optional improvements.
