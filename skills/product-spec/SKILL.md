...
6. **Publish and lock.** Create or update the Product Contract in the configured project tracker. Use native workflow states and parent relationships when available; otherwise retain the metadata in the body. Record the approving person and date only when evidenced.
